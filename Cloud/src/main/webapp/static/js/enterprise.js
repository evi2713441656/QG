...
function createDepartment() {
    ...
    if (!name) {
        UIController.showToast('请输入部门名称', 'error');
        return;
    }

    UIController.showLoading();

    fetch('/enterprise/department', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        ...
        .then(data => {
            if (data.code === 200) {
                UIController.showToast('创建部门成功', 'success');
                hideCreateDepartmentModal();
                loadDepartments(currentEnterpriseId);
            } else {
                UIController.showToast(data.message || '创建部门失败', 'error');
            }
        })
        .catch(err => {
            UIController.showToast('创建部门失败', 'error');
...
function loadDepartments(enterpriseId) {
    UIController.showLoading();

    fetch(`/enterprise/${enterpriseId}/departments`)
        .then(response => response.json())
        .then(data => {
            if (data.code === 200 && data.data) {
                ...
                if (data.data.length === 0) {
                    container.innerHTML = '<div class="text-center text-gray-500 p-4">暂无部门</div>';
                    return;
                }

                data.data.forEach(dept => {
                    container.appendChild(createDepartmentCard(dept));
                });
            } else {
                UIController.showToast(data.message || '加载部门列表失败', 'error');
            }
        })
        .catch(err => {
            UIController.showToast('加载部门列表失败', 'error');
...
function createDepartmentCard(department) {
    const card = document.createElement('div');
    card.className = 'department-card bg-white shadow-md rounded p-4 mb-4';

    card.innerHTML = `
    <div class="flex justify-between items-center">
      <h3 class="text-lg font-bold">${department.name}</h3>
      <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">成员: ${department.memberCount || 0}</span>
    </div>
    <p class="text-gray-600 mt-2">${department.description || '无描述'}</p>
    <p class="text-gray-600">部门主管: ${department.managerName || '未指定'}</p>
    <div class="mt-4 flex space-x-2">
      <button class="bg-blue-500 text-white px-3 py-1 rounded" onclick="showDepartmentDetail(${department.id})">
        查看详情
      </button>
      ${getCurrentUserRoleInEnterprise() <= 2 ? `
      <button class="bg-yellow-500 text-white px-3 py-1 rounded" onclick="showEditDepartmentModal(${department.id})">
        编辑
      </button>
      <button class="bg-red-500 text-white px-3 py-1 rounded" onclick="confirmDeleteDepartment(${department.id})">
        删除
      </button>
      ` : ''}
    </div>
...
function clockIn() {
    ...
    fetch('/enterprise/attendance/clock-in', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        ...
        .then(data => {
            if (data.code === 200) {
                UIController.showToast('上班打卡成功', 'success');
                updateAttendanceStatus();
            } else {
                UIController.showToast(data.message || '打卡失败', 'error');
            }
        })
        .catch(err => {
            UIController.showToast('打卡失败', 'error');
...
function clockOut() {
    ...
    fetch('/enterprise/attendance/clock-out', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        ...
        .then(data => {
            if (data.code === 200) {
                UIController.showToast('下班打卡成功', 'success');
                updateAttendanceStatus();
            } else {
                UIController.showToast(data.message || '打卡失败', 'error');
            }
...
function updateAttendanceStatus() {
    fetch(`/enterprise/${currentEnterpriseId}/attendance/status`)
        .then(response => response.json())
        .then(data => {
            if (data.code === 200 && data.data) {
...
function publishEnterpriseNotice() {
    ...
    const title = document.getElementById('notice-title').value;
    const content = document.getElementById('notice-content').value;
    const isPrivate = document.getElementById('notice-is-private').checked;

    // 获取选中的成员ID列表
    const selectedMembers = Array.from(document.querySelectorAll('#notice-recipients input[type="checkbox"]:checked'))
    ...
    if (isPrivate && selectedMembers.length === 0) {
        UIController.showToast('请选择至少一名通知接收者', 'error');
        return;
    }
    ...
    fetch('/enterprise/notice', {
        ...
        body: JSON.stringify({
            ...
            content,
            isPrivate,
            recipients: isPrivate ? selectedMembers : []
        })
    })
        .then(response => response.json())
        .then(data => {
